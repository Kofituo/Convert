...
import com.otuolabs.unitconverter.subclasses.FavouritesAdapter.Companion.favouritesAdapter
...
class FavouritesActivity : AppCompatActivity(), FavouritesAdapter.FavouritesItem,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        motionLayout?.apply {
            if (viewModel.favouritesProgress == 0f)
                transitionToStart()
    ...
    override fun onQueryTextChange(newText: String?): Boolean {
        if (!::favouritesAdapter.isInitialized || newText.isNull()) return true
        if (newText.isEmpty()) {
