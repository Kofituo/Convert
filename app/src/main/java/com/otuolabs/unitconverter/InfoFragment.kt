...
class InfoFragment : DialogFragment() {
    ...
    private fun aboutQuantity(): AboutQuantity? {
        ...
        return when (arguments?.getInt("viewId")) {
            ...
            R.id.sound -> {
                unit = getUnits(Sound(requireContext()).getList())
                AboutQuantity(
                        R.string.sound.gS,
                        R.string.sound_meta.gS,
                        R.string.sound_instr.gS,
                        unit, didYouKnow
                )
            }

            R.id.resistance -> {
                unit = getUnitsWithPrefix(Resistance(requireContext()).getList(), 17)
                AboutQuantity(
                        R.string.resistance.gS,
                        R.string.resistance_meta.gS,
                        R.string.resistance_instr.gS,
                        unit, didYouKnow
                )
            }

            R.id.radioactivity -> {
                list = Radioactivity(requireContext()).getList()
                unit = appendString {
                    add { getUnits(list.subList(0, 1)) }
                    addWithSpace { ", " }
                    add { getUnits(list.subList(13, 14)) }
                    add { ", " }
                    add { getUnits(list.subList(20, 21)) }
                }
                AboutQuantity(
                        R.string.radioactivity.gS,
                        R.string.radio_def.gS,
                        R.string.radio_instr.gS,
                        unit, didYouKnow
                )
            }
            R.id.force -> {
                unit = getUnitsWithPrefix(Force(requireContext()).getList(), 17)
                AboutQuantity(
                        R.string.force.gS,
                        R.string.force_meta.gS,
                        R.string.force_instr.gS,
                        unit, didYouKnow
                )
            }
            R.id.power -> {
                unit = getUnitsWithPrefix(Power(requireContext()).getList(), 17)
                AboutQuantity(
                        R.string.power.gS,
                        R.string.power_meta.gS,
                        R.string.power_instr.gS,
                        unit, didYouKnow
                )
            }

            R.id.density -> {
                list = Density(requireContext()).getList()
                unit = appendString {
                    add { addOne(list[0]) }
                    add { getUnits(list.subList(6, 7)) }
                    add { ", " }
                    add { getUnits(list.subList(14, list.size)) }
                }
                AboutQuantity(
                        R.string.density.gS,
                        R.string.density_meta.gS,
                        R.string.density_instr.gS,
                        unit, didYouKnow
                )
            }
            R.id.flow -> {
                list = Flow(requireContext()).getList()
                unit = appendString {
                    add { getUnits(list.subList(0, 3)) }
                    add { ", " }
                    add { getUnits(list.subList(9, 12)) }
                    add { ", " }
                    add { getUnits(list.subList(15, list.size)) }
                }
                AboutQuantity(
                        R.string.flow.gS,
                        R.string.flow_meta.gS,
                        R.string.flow_instr.gS,
                        unit, didYouKnow
                )
            }
            R.id.inductance -> {
                unit = getUnitsWithPrefix(Inductance(requireContext()).getList(), 6)
                AboutQuantity(
                        R.string.inductance.gS,
                        R.string.inductance_def.gS,
                        R.string.inductance_instr.gS,
                        unit, didYouKnow
                )
            }
            R.id.resolution -> {
                unit = getUnits(Resolution(requireContext()).getList().subList(0, 2))
                AboutQuantity(
                        R.string.resolution.gS,
                        R.string.resolution_meta.gS,
                        R.string.data_instr.gS,
                        unit, didYouKnow
                )
            }
            R.id.cooking -> {
                unit = getUnits(Cooking(requireContext()).getList())
                val instruments = appendString {
                    add { getString(R.string.coffee_spoon) }
                    add { ", " }
                    add { getString(R.string.teaspoon) }
                    add { ", " }
                    add { getString(R.string.tablespoon) }
                    add { ", " }
                    add { getString(R.string.dessertspoon) }
                    add { ", " }
                    add { getString(R.string.cup) }
                }
                AboutQuantity(
                        R.string.cooking.gS,
                        R.string.cooking_meta.gS,
                        instruments, unit, didYouKnow
                )
            }
            R.id.number_base -> {
                unit = getUnits(NumberBase(requireContext()).getList())
                AboutQuantity(
                        R.string.number_base.gS,
                        R.string.number_base_meta.gS,
                        null, unit, didYouKnow
                )
            }
            else -> null
        }
